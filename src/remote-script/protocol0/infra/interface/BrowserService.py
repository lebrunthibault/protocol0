...
class BrowserService(BrowserServiceInterface):
    ...
    def load_device(self, name: str) -> Sequence:
        ...
        if (
            name.endswith(".adv")
            or name.endswith(".adg")
            or name.endswith(".vstpreset")
        ):
            load_func = partial(self._browser_loader_service.load_from_user_library, name)
        else:
            load_func = partial(self._browser_loader_service.load_device, name)
