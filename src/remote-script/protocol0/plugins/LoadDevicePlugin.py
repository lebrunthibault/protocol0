"""Load a Live device by name — a plugin made of a single ``@action``.

This is a real, enabled action (not a template): it powers the ``load_device``
shortcut. It also demonstrates the **single-file drop-in** form — a plugin can be
one ``.py`` file dropped directly under ``plugins/``, with no package or
``__init__.py``.
"""
...
class LoadDevicePlugin(PluginInterface):
    name = "load_device"
